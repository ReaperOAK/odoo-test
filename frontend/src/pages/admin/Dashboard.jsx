...
const AdminDashboard = () => {
  ...
  const exportOrderData = async () => {
    try {
      ...
      csv += `Generated at: ${new Date().toLocaleString()}\n\n`;
      csv +=
        "Order ID,Customer Name,Customer Email,Host Name,Listing Title,Amount,Status,Payment Status,Order Date,Start Date,End Date\n";
      ...
      orderData.forEach((order) => {
        ...
        csv += `"${
          order.hostId?.name || order.hostId?.hostProfile?.displayName || "N/A"
        }",`;
        ...
        csv += `"${
          order.lines?.[0]?.start
            ? new Date(order.lines[0].start).toLocaleDateString()
            : "N/A"
        }",`;
        csv += `"${
          order.lines?.[0]?.end
            ? new Date(order.lines[0].end).toLocaleDateString()
            : "N/A"
        }"\n`;
      });
  ...
  const exportRevenueReport = async () => {
    try {
      ...
      analyticsDataResponse.revenueOverTime?.forEach((item) => {
        ...
        csv += `"${dateStr}","₹${item.revenue?.toLocaleString() || 0}","${
          item.orders || 0
        }"\n`;
      });
      ...
      categories.forEach((category) => {
        const percentage =
          totalBookings > 0
            ...
            : 0;
        csv += `"${category._id || "Unknown"}","${category.bookings || 0}","₹${
  ...
  const exportPayoutReport = async () => {
    try {
      ...
      csv += `Generated at: ${new Date().toLocaleString()}\n\n`;
      csv +=
        "Host Name,Host Email,Amount,Currency,Status,Method,Created Date,Updated Date,Bank Account,IFSC,Account Holder\n";
