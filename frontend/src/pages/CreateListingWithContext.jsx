...
const CreateListingWithContext = () => {
  ...
  const validateForm = () => {
    ...
    else if (formData.location.length < 2) errors.location = 'Location must be at least 2 characters';
    else if (formData.location.length > 200) errors.location = 'Location must be less than 200 characters';

    if (!formData.totalQuantity) errors.totalQuantity = 'Quantity is required';
    else if (isNaN(formData.totalQuantity) || parseInt(formData.totalQuantity) <= 0) {
      errors.totalQuantity = 'Quantity must be a positive number';
    }

    if (!formData.depositValue) errors.depositValue = 'Deposit value is required';
    else if (isNaN(formData.depositValue) || parseFloat(formData.depositValue) < 0) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-6">
            You need to be a host to create listings. Please upgrade your account to start renting out your items.
          </p>
        ...
        <Card.Content className="p-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={handleInputChange}
                    placeholder="1"
                    className={validationErrors.totalQuantity ? 'border-red-300' : ''}
                  />
                  {validationErrors.totalQuantity && (
                    <p className="text-red-600 text-sm mt-1">{validationErrors.totalQuantity}</p>
                  )}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label htmlFor="depositType" className="block text-sm font-medium text-gray-700 mb-1">
                    Deposit Type *
                  </label>
                  <select
                    id="depositType"
                ...
                <div>
                  <label htmlFor="depositValue" className="block text-sm font-medium text-gray-700 mb-1">
                    Deposit Value * {formData.depositType === 'percent' ? '(%)' : '(₹)'}
                  </label>
                  <Input
                    id="depositValue"
                    name="depositValue"
                    ...
                    className={validationErrors.depositValue ? 'border-red-300' : ''}
                  />
                  {validationErrors.depositValue && (
                    <p className="text-red-600 text-sm mt-1">{validationErrors.depositValue}</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                />
                <p className="text-sm text-gray-500 mt-1">
                  Add URLs of images showing your item (optional but recommended)
                </p>
              </div>
            </div>
